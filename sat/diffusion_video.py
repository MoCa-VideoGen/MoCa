...
class SATVideoDiffusionEngine(nn.Module):
    ...
    def forward(self, x, batch):
        ...
        #/sat/sgm/modules/diffusionmodules/loss.py P82
        loss_mean = loss.mean()
    ...
    def sample(
        ...
        batch_size: int = 16,
        shape: Union[None, Tuple, List] = None,  # (T, C, H // F, W // F)
        prefix=None,
        concat_images=None,
        **kwargs,
    ):
        ...
        camera_traj = kwargs.get("camera_traj")
        text = kwargs.get("text")
        #self.model:OpenAIWrapper
        denoiser = lambda input, input_reference, sigma, c, **addtional_model_inputs: self.denoiser( 
            self.model, input, input_reference, sigma, c, concat_images=concat_images, video_flow=video_flow, **addtional_model_inputs
        )

        samples, sample_reference = self.sampler(denoiser, randn, cond, uc=uc, scale=scale, scale_emb=scale_emb,camera_traj=camera_traj,text=text)
        samples = samples.to(self.dtype)
        sample_reference = sample_reference.to(self.dtype)
        return samples, sample_reference 
