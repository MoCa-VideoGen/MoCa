...
class VideoDiffusionLoss(StandardDiffusionLoss):
    ...
    def __call__(self, network, denoiser, conditioner, input, batch):  #看看input里面有什么
        cond = conditioner(batch) #Tora_Camera/sat/sgm/modules/encoders/modules.py P185
        additional_model_inputs = {key: batch[key] for key in self.batch2model_keys.intersection(batch)}
        ...
        #torch.distributed.broadcast(alphas_cumprod_sqrt, src=0, group=dp_group)

        #dp_group = mpu.get_data_parallel_group()
        #torch.distributed.broadcast(idx, src=0, group=dp_group)

        additional_model_inputs["idx"] = idx
        #print(f"Rank {torch.distributed.get_rank()}: idx={idx}")
        #print(f"Rank {torch.distributed.get_rank()}: alphas_cumprod_sqrt[:5] = {alphas_cumprod_sqrt[:5]}")
        #print(f"Rank {torch.distributed.get_rank()}: noise_sum={noise.sum().item()}, noise_mean={noise.mean().item()}")
        additional_model_inputs["video_flow"] = batch["video_flow"]  #None
        additional_model_inputs["camera_traj"] =batch["camera_traj"]
        additional_model_inputs['txt'] = batch['txt']
        #print(f"batch[cam] is {additional_model_inputs['camera_traj'].shape}")
        if self.offset_noise_level > 0.0:
            noise = (
                noise + append_dims(torch.randn(input.shape[0]).to(input.device), input.ndim) * self.offset_noise_level
            )
        ...
        noised_input_reference = input.float() * append_dims(alphas_cumprod_sqrt, input.ndim) + noise * append_dims(
            (1 - alphas_cumprod_sqrt**2) ** 0.5, input.ndim
        )
        noised_input_reference = noised_input_reference.detach()

        model_output, _ = denoiser(network, noised_input, noised_input_reference, alphas_cumprod_sqrt, cond, **additional_model_inputs)
        #/Tora_Camera/sat/sgm/modules/diffusionmodules/denoiser.py P36
