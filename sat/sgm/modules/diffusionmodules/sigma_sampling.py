...
class DiscreteSampling:
    def __init__(self, discretization_config, num_idx, do_append_zero=False, flip=True, uniform_sampling=False):
        ...
        self.sigmas = instantiate_from_config(discretization_config)(num_idx, do_append_zero=do_append_zero, flip=flip).to("cuda")
    ...
    def __call__(self, n_samples, rand=None, return_idx=False):
        if self.uniform_sampling:
            ...
            # 主 GPU 生成随机 idx，并广播到组内其他 GPU
            if dp_rank == 0:
                idx = torch.randint(0, self.num_idx, (n_samples,),device=self.sigmas.device)
            else:
                idx = torch.empty((n_samples,), dtype=torch.long,device=self.sigmas.device)
            
            # 在数据并行组内广播 idx
            torch.distributed.broadcast(idx, src=0, group=dp_group)

        #with open("/lpai/volumes/base-3da-ali-sh/chengzhijing/gv8s/Tora_Camera/sigma1.txt","a") as f:
        #    f.write(f"rank is {rank} sigma is {idx}\n")
        if return_idx:
