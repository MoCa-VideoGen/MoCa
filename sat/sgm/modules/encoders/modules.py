...
class GeneralConditioner(nn.Module):
    ...
    def get_single_embedding(
        ...
        with embedding_context():
            if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                if embedder.legacy_ucg_val is not None: #不做
                    if cond_or_not is None:
                        batch = self.possibly_get_ucg_val(embedder, batch) #batch=['txt':'A man is..']
                    else:
                        batch = self.surely_get_ucg_val(embedder, batch, cond_or_not)
                emb_out = embedder(batch[embedder.input_key]) #最好输出看看output形状 走264行
        ...
        if not isinstance(emb_out, (list, tuple)):
            emb_out = [emb_out]
        for emb in emb_out:
            out_key = self.OUTPUT_DIM2KEYS[emb.dim()] #设置不同的操作方式
            ...
            if out_key in output: #crossattn
    ...
    def get_unconditional_conditioning(self, batch_c, batch_uc=None, force_uc_zero_embeddings=None): #force_uc_zero_embeddings : 文本嵌入器在生成无条件嵌入时应输出零向量
        if force_uc_zero_embeddings is None:
            force_uc_zero_embeddings = []
        ucg_rates = list()
        for embedder in self.embedders: #FrozenT5Embedder len = 1 
            ucg_rates.append(embedder.ucg_rate)  #[0.1]
        ...
        c = self(batch_c) # c['crossattn'].shape = [B,226,4096]
...
class FrozenT5Embedder(AbstractEmbModel):
    ...
    def forward(self, text):
        batch_encoding = self.tokenizer(
            text,
            truncation=True,
            max_length=self.max_length, #226
            return_length=True,
