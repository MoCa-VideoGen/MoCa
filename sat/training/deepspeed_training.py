...
def training_main(args, model_cls, forward_step_function, create_dataset_function, handle_metrics_function=None, init_function=None, collate_fn=None, forward_step_eval=None):
    ...
    # modules/SwissArmyTransformer/sat/data_utils/configure_data.py P177

    if args.epochs:
        args.train_iters = len(train_data)  #15000
    ...
    if args.train_iters > 0:
        ...
        # 查看模型中每个参数的梯度状态
        #with open('/mnt/pfs-mc0p4k/cvg/team/czj/czj/Tora_Camera/sat/grad.txt', 'w') as f:
        ...
        for name, param in model.named_parameters():
            if id(param) in seen_params:
                print(f"Parameter {name} is shared!")
            seen_params.add(id(param))
        #print("****")
        print_rank0("加载V9从4000次开始")
        #print_rank0("lr改为原来的00001")
        summary_writer = None
...
def setup_model_untrainable_params_and_optimizer(args, model, config_params=None):
    ...
    if hasattr(model, 'disable_untrainable_params'):
       # 在训练开始前打印所有模块名称
        ...
        #    if "camera_insert" in name:
        #        print(f"Found camera_insert module: {name}")
    ...
    if args.train_data is not None:
        if args.deepspeed:
            ...
            model, optimizer, _, _ = deepspeed.initialize( #/mnt/pfs-mc0p4k/cvg/team/czj/czj/Tora_Camera/modules/SwissArmyTransformer/sat/mpu/initialize.pyP168
                model=model,
...
def train(model, optimizer, lr_scheduler,
    ...
    print(f"[RANK {rank}/{world_size}] Hello from node_rank={os.environ.get('NODE_RANK')} local_rank={local_rank}", flush=True)

    """Train the model."""
    if train_data is not None:
        train_data_iterator = iter(train_data)  #data_video/get_data P383
    else:
    ...
    while args.iteration < args.train_iters: #0-15000
...
def train_step(data_iterator, model, optimizer, lr_scheduler,
    ...
    #print(f"Rank {torch.distributed.get_rank()} is running forward_step")
    ...
    while True:
        ...
        #Tora_Camera/sat/train_video.py P366
        ...
        if torch.isnan(lm_loss).any() or torch.isinf(lm_loss).any():
            print(f" NaN detected in lm_loss: {lm_loss}")
