...
class SelfAttention(torch.nn.Module):
    ...
    def forward(self, hidden_states, mask, *args, **kw_args):
        if 'attention_forward' in self.hooks:
            ...
            #做 modules/SwissArmyTransformer/sat/transformer_defaults.py P82
...
class BaseTransformer(torch.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask, *,
        ...
        if 'word_embedding_forward' in self.hooks: 
            hidden_states, hidden_states_reference = self.hooks['word_embedding_forward'](input_ids, output_cross_layer=output_cross_layer, **kw_args)
           
        else:  # default
            hidden_states, hidden_states_reference = HOOKS_DEFAULT['word_embedding_forward'](self, input_ids, output_cross_layer=output_cross_layer,**kw_args)
        ...
        hidden_states_reference = self.embedding_dropout(hidden_states_reference)
  
        
        output_per_layers = []
        output_per_layers_reference = []
        if self.checkpoint_activations:
            def custom(start, end, kw_args_index, cross_layer_index):
                def custom_forward(*inputs): 
                    layers_ = self.layers[start:end]
                    x_, hidden_states_reference, mask = inputs[0], inputs[1], inputs[2]
                    ...
                    for i, layer in enumerate(layers_):
                        ...
                        if 'layer_forward' in self.hooks:
                            layer_ret,hidden_states_reference = self.hooks['layer_forward'](  #dit主函数
                                x_, hidden_states_reference,mask, layer_id=layer.layer_id,
                            ...
                            layer_ret,hidden_states_reference = layer(
                                x_, hidden_states_reference, mask, layer_id=layer.layer_id,
                                **kw_args, position_ids=position_ids, **output_cross_layer,
                                output_this_layer=output_this_layer_obj,
                                output_cross_layer=output_cross_layer_obj,
                        ...
                        if isinstance(layer_ret, tuple):
                            ...
                            layer_ret_reference = layer_ret_reference[0]
                        x_, hidden_states_reference, output_this_layer, output_cross_layer = layer_ret, hidden_states_reference, output_this_layer_obj, output_cross_layer_obj   #output_this_layer_obj:{} output_cross_layer_obj:{}
                    ...
                    return (x_, hidden_states_reference, output_per_layers_part, output_cross_layer, *flat_outputs)
            ...
            if self.training:
                ...
                hidden_states_reference.requires_grad_(False)
            l, num_layers = 0, len(self.layers)
            chunk_length = self.checkpoint_num_layers
            output_this_layer = []
            while l < num_layers: 
               
                args = [hidden_states, hidden_states_reference, attention_mask]
                ...
                if l + self.checkpoint_skip_layers >= num_layers:
                    ...
                    hidden_states, hidden_states_reference, output_per_layers_part, output_cross_layer, *flat_outputs = \
                ...
                output_per_layers_reference.append(output_per_layers_part)
                l += chunk_length

        else: 
            output_this_layer = []
            output_this_layer_reference = []
            for i, layer in enumerate(self.layers):
                ...
                if 'layer_forward' in self.hooks: 
                    layer_ret, layer_ret_reference = self.hooks['layer_forward'](*args, #dit_concat主函数
                    ...
                    layer_ret, layer_ret_reference = layer(*args, layer_id=torch.tensor(i), **kw_args, position_ids=position_ids, **output_cross_layer,
                        output_this_layer=output_this_layer_obj, output_cross_layer=output_cross_layer_obj)
                if isinstance(layer_ret, tuple):
                    layer_ret = layer_ret[0]  # for legacy API
                    layer_ret_reference = layer_ret_reference[0]
                hidden_states, hidden_states_reference, output_this_layer, output_cross_layer = layer_ret, layer_ret_reference, output_this_layer_obj, output_cross_layer_obj
                if output_hidden_states:
                    output_this_layer['hidden_states'] = hidden_states
                    output_this_layer_reference['hidden_states'] = hidden_states_reference
                output_per_layers.append(output_this_layer)
                output_per_layers_reference.append(output_this_layer_reference)
        ...
        if self.use_final_layernorm:
            ...
            logits_reference = self.final_layernorm(hidden_states_reference)
        else:
            logits = hidden_states
            logits_reference = hidden_states_reference
          

        logits = copy_to_model_parallel_region(logits)
        logits_reference = copy_to_model_parallel_region(logits_reference)
        if 'final_forward' in self.hooks:
            ...
            logits_parallel_reference = self.hooks['final_forward'](logits_reference, **kw_args, parallel_output=self.parallel_output)
        else:
            logits_parallel = HOOKS_DEFAULT['final_forward'](self, logits, **kw_args, parallel_output=self.parallel_output)
            logits_parallel_reference = HOOKS_DEFAULT['final_forward'](self, logits_reference, **kw_args, parallel_output=self.parallel_output)
        
        outputs = [logits_parallel]
        outputs.extend(output_per_layers)
        outputs_reference = [logits_parallel_reference]
        outputs_reference.extend(output_per_layers_reference)
        
        return outputs, outputs_reference
