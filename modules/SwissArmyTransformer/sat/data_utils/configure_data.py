...
def make_dataset_full(path, split, args, create_dataset_function, 
    ...
    if split is None:
        split = [1.] 
    if not should_split(split): #做
        ds = []  #/sat/data_video.py P364
        for p in path:  #p:训练集路径
            d = create_dataset_function(p, args) #SFTDataset
            ds.append(d)
        ds = ConcatDataset(ds, weights=dataset_weights) # #P329
        if random_mapping:
...
def make_loaders(args, create_dataset_function, collate_fn=None):
    ...
    if train is not None and args.batch_size > 0:  #train:<sat.data_utils.configure_data.RandomMappingDataset object 
