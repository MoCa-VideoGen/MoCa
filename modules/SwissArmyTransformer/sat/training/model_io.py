...
def save_checkpoint(iteration, model, optimizer,
    ...
    if hasattr(args, 'deepspeed') and args.deepspeed:
        ...
        if optimizer is not None and optimizer.optimizer.__class__.__name__ ==  "FusedEmaAdam" :
            update_ema_parameters_to_model(optimizer) #将当前的 EMA 权重更新到模型中 
            #使用 EMA 技术平滑过的模型权重。相比于常规的训练模型，EMA 模型通常能提供更加稳定和泛化能力更强的结果
            if mpu.get_data_parallel_rank() == 0:
...
def save_ds_checkpoint(iteration, model, lr_scheduler, args, use_ema = False):
    """Save a model checkpoint."""
    #sd:dict，存储了训练中需要保存的关键信息，如训练迭代数、学习率调度器状态和随机数生成器状态
    sd = {}
    sd['iteration'] = iteration
    if lr_scheduler is not None:
        sd['client_lr_scheduler'] = lr_scheduler.state_dict()
    # rng states.
    if not args.no_save_rng: #保存随机数生成器状态
...
'''
def save_ds_checkpoint(iteration, model, optimizer, lr_scheduler, args, use_ema=False):
    """Save DeepSpeed checkpoint with optimizer states."""
    sd = {
        'iteration': iteration,
        'client_lr_scheduler': lr_scheduler.state_dict() if lr_scheduler else None
    }
    ...
    # 使用DeepSpeed的save_checkpoint方法保存模型和优化器状态
    tag = str(iteration) + ('-ema' if use_ema else '')
    model.save_checkpoint(
        save_dir=args.save,
        tag=tag,
        client_state=sd,
        save_latest=(not use_ema)  # 通常只保存最新的非EMA检查点
    )
'''
...
def load_checkpoint(model, args, load_path=None, prefix='', specific_iteration=None):
    ...
    # if given `prefix`前缀, load a speficic prefix in the checkpoint, e.g. encoder
    new_sd = {'module':{}}
    for k in sd:
        if k != 'module':
            new_sd[k] = sd[k]
    ...
    if len(missing_keys) > 0:
        if args.mode == 'inference':
            ...
            if not args.force_train:  #是否强制训练
            ...
            for key_name in missing_keys:
                ...
                #if 'camera_insert' in parts:
                #    mixin_name = parts[parts.index('adaln_layer')+1]
                #if mixin_name=='camera_insert' and mixin_name not in mixin_names:
                #    mixin_names.append(mixin_name)
...
def load_checkpoint(model, args, load_path=None, prefix='', specific_iteration=None):
    ...
    load_path, client_state = model.load_checkpoint(
        args.load,  # 检查点目录
        tag=iteration,  # 检查点标签
        load_optimizer_states=True,  # 加载优化器状态
        load_lr_scheduler_states=True,  # 加载学习率调度器状态
        load_module_only=False,  # 加载完整状态（模型+优化器+调度器）
        load_module_strict=False  # 允许部分加载
    )
    # 在加载检查点后恢复调度器状态
    if lr_scheduler is not None and client_state is not None:
        if 'client_lr_scheduler' in client_state:
            lr_scheduler.load_state_dict(client_state['client_lr_scheduler'])
        else:
            print_rank0("Warning: No lr_scheduler state found in checkpoint.")
    # if given `prefix`前缀, load a speficic prefix in the checkpoint, e.g. encoder
    new_sd = {'module':{}}
    for k in sd:
        if k != 'module':
